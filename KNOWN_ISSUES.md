...
import of `gpu_benchmark/gpu_bench.py`. Nothing on the live hot path
(`fact_sheet.py::build_fact_sheet()` -> `verify.roll_maia()`) ever puts
`gpu_benchmark/` on `sys.path`, and `roll_maia()` wraps the whole call in
...
anywhere in `chess_plans/` or its callers. `gpu_benchmark/` is documented
...
the package doesn't reach into `gpu_benchmark/`'s batch-job-oriented
