"""backward_push_audit — three-leg audit of the NEW backward_push family
(2026-07-22 user request: "pushing the backward pawn to get rid of the
weakness — surface it and see if engine lines do it").

The rule (plan_diff.py): t pushes a pawn that stood in t's
backward_half_open set (non-capture), and t's backward count DROPS and
...
Leg 2  ENGINE LINES (banked eng_shards + benchmark_v1; ZERO new engine
       ...
       backward_push fire in an eval-equal engine PV (band 50cp, the
       reduce_agreement convention)? Compare vs the ACTUAL continuation
       at the same positions.

Run: python3 experiments/backward_push_audit.py [--corpus-games N]
"""
...
ENG = "/Users/avismara/Development/lucena/lucena-plans/experiments/eng_shards"
ANCHORS = (20, 30, 40, 50)
H_FAST, T_FAST = 12, 2
H_SLOW, T_SLOW = 25, 6
EQUAL_BAND = 50
...
def fires(b, ucis_or_moves, h, t):
    ...
    return {lab.split(":")[0] for lab in labels(b, mvs, h, t)
            if lab.split(":")[1] == "backward_push"}


def _sides_with_backward(b):
    out = []
    if backward_half_open(b, chess.WHITE):
        out.append("W")
    if backward_half_open(b, chess.BLACK):
        out.append("B")
    return out
...
def leg2():
    ...
    print(f"  benchmark positions with a backward pawn: {n_pre}")
    print(f"  backward_push fires in an eval-equal engine PV: "
